  ...
  try {
    ...
    if (!file || !(file instanceof File)) {
      return NextResponse.json(
        { error: "Story media file is required." },
        { status: 400 }
      );
    }
    ...
    if (
      ...
      return NextResponse.json(
        { error: "Story upload is not configured." },
        { status: 500 }
      );
    }
    ...
    uploadData.append("folder", "buildup/community-stories");
    ...
    if (!uploadRes.ok || !data.secure_url) {
      console.error("COMMUNITY STORY MEDIA UPLOAD ERROR:", data);
    ...
    console.error("COMMUNITY STORY UPLOAD API ERROR:", error);
